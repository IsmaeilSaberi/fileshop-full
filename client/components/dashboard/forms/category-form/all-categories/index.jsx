...
const AllCategories = ({
  setCategoryDetailCtrl,
  setRandNumForCategoryClick,
}) => {
  ...
  const [allCategoriesNumbers, setAllCategoriesNumbers] = useState(0);
  const paginate = 10;
  ...
  useEffect(() => {
    axios
      .get(
        `https://fileshop-server.iran.liara.run/api/categories?pn=${pageNumber}&&pgn=${paginate}`,
        {
      ...
      .then((d) => {
        ...
        setBtnNumbers([
          ...Array(Math.ceil(d.data.AllCategoriesNumber / paginate)).keys(),
        ]);
        setAllCategoriesNumbers(d.data.AllCategoriesNumber);
      })
      .catch((err) => {
        ...
        {filteredBtns[0] == -1 ? (
          ...
          filteredBtns.map((n, i) => (
            <button
              ...
              onClick={() => {
                n + 1 == pageNumber ? console.log("") : setCategories([-1]);
                setPageNumber(n + 1);
                goToTop();
              }}
              key={i}
